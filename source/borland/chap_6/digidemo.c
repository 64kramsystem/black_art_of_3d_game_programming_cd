...
_sound ones[11];   // this will hold the digital samples for 1..9
_sound teens[11];  // this will hold the digital samples for 11-19
_sound tens[11];   // this will hold the digital samples for 10,20,30,40...100
_sound phrases[6]; // this will hold the phrases
...
if (number<10)
   {
   Sound_Play((_sound_ptr)&ones[number]);
...
if (number >= 11 && number <= 19)
   {
   Sound_Play((_sound_ptr)&teens[number-11]);
   while(Sound_Status()==1);
   return;
   } // end if 11..19
...
Sound_Play((_sound_ptr)&tens[tens_place-1]);
while(Sound_Status()==1);
...
if (ones_place)
   {
   Sound_Play((_sound_ptr)&ones[ones_place]);
   while(Sound_Status()==1);
	} // end if
...
for (number=1; number<=9; number++)
    ...
    Sound_Load(filename,(_sound_ptr)&ones[number],1);
...
for (number=11; number<=19; number++)
    ...
    Sound_Load(filename,(_sound_ptr)&teens[number-11],1);
...
for (number=10; number<=100; number+=10)
    ...
    // load the sound
	 ...
	 Sound_Load(filename,(_sound_ptr)&tens[-1+number/10],1);
...
Sound_Load("what.voc",   (_sound_ptr)&phrases[SOUND_WHAT     ],1);
Sound_Load("wrong.voc",  (_sound_ptr)&phrases[SOUND_WRONG    ],1);
Sound_Load("correct.voc",(_sound_ptr)&phrases[SOUND_CORRECT  ],1);
Sound_Load("plus.voc",   (_sound_ptr)&phrases[SOUND_PLUS     ],1);
Sound_Load("equal.voc",  (_sound_ptr)&phrases[SOUND_EQUAL    ],1);
...
while(!done)
     ...
     Sound_Play((_sound_ptr)&phrases[SOUND_WHAT]);
     while(Sound_Status()==1);
     ...
     Sound_Play((_sound_ptr)&phrases[SOUND_PLUS]);
     while(Sound_Status()==1);
     ...
     printf(" = ?",number_2);
	  ...
	  Sound_Play((_sound_ptr)&phrases[SOUND_EQUAL]);
	  while(Sound_Status()==1);
     ...
     if (answer!=0)
        ...
        if (answer == (number_1 + number_2))
           {
			  Sound_Play((_sound_ptr)&phrases[SOUND_CORRECT]);
			  while(Sound_Status()==1);
			  num_correct++;
			  }
		  else
           ...
           Sound_Play((_sound_ptr)&phrases[SOUND_WRONG]);
           while(Sound_Status()==1);
...
for (number=1; number<=9; number++)
    Sound_Unload((_sound_ptr)&ones[number]);

for (number=11; number<=19; number++)
    Sound_Unload((_sound_ptr)&teens[number]);

for (number=10; number<=100; number+=10)
	 Sound_Unload((_sound_ptr)&tens[-1+number/10]);

for (number=0; number<=4; number++)
	 Sound_Unload((_sound_ptr)&phrases[number]);
