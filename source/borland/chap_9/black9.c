...
serial_ch = inp(open_port + SERIAL_RBF);
...
outp(port_base + SERIAL_LCR, SERIAL_DIV_LATCH_ON);

// send low and high bytes to divsor latches

outp(port_base + SERIAL_DLL, baud);
outp(port_base + SERIAL_DLH, 0);

// set the configuration for the port

outp(port_base + SERIAL_LCR, configuration);

// enable the interrupts

data = inp(port_base + SERIAL_MCR);
data = SET_BITS(data,SERIAL_GP02);
outp(port_base + SERIAL_MCR, data);

outp(port_base + SERIAL_IER, 1);
...
data = inp(open_port + SERIAL_MCR);
data = RESET_BITS(data,SERIAL_GP02);

outp(open_port + SERIAL_MCR, data);

outp(open_port + SERIAL_IER, 0);
